

variable "vmw_enable_vcf_mode" {
  
}

variable "vmw_resource_group_id" {
  
}


variable "vmw_host_count" {
  
}


variable "vmw_host_profile" {

}

variable "vmw_esx_image" {

}

variable "vmw_vpc_zone" {
  
}

variable "vmw_host_subnet" {
  
}

variable "vmw_mgmt_subnet" {
  
}

variable "vmw_vmot_subnet" {
  
}

variable "vmw_vsan_subnet" {
  
}

variable "vmw_tep_subnet" {
  
}

variable "vmw_vpc" {
  
}

variable "vmw_key" {
  
}

variable "vmw_instance_ssh_private_key" {
  
}


variable "vmw_resources_prefix" {
  
}


variable "vmw_dns_root_domain" {
  
}

variable "vmw_cluster_prefix" {
  
}


variable "vmw_sg_mgmt" {
  
}


variable "vmw_sg_vmot" {
  
}


variable "vmw_sg_vsan" {
  
}

variable "vmw_sg_tep" {
  
}


###

variable "vmw_mgmt_vlan_id" {

}

variable "vmw_vmot_vlan_id" {

}

variable "vmw_vsan_vlan_id" {

}

variable "vmw_tep_vlan_id" {

}

variable "vmw_edge_uplink_public_vlan_id" {

}

variable "vmw_edge_uplink_private_vlan_id" {

}

